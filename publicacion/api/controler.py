...
class putPublicacion(APIView):
    def put(self,request, id):
        try:
            publicacion = serializable.models.publicacion.objects.get(pk=id)
            serializer = serializable.PublicacionSerializable(instance=publicacion,data=request.data)
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data, status = status.HTTP_200_OK)
            return Response(serializer.errors, status = status.HTTP_400_BAD_REQUEST)        
...
class deletePublicacion(APIView):
    def delete(self,request,id):
        try:
            publicacion = serializable.models.publicacion.objects.get(pk=id)
            publicacion.delete()
            return Response("Publicacion eliminada correctamente", status = status.HTTP_200_OK) 
        except Exception as e:   
            return Response(str(e), status = status.HTTP_500_INTERNAL_SERVER_ERROR)